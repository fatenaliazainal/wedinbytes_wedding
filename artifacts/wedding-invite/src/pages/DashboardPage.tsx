...
import logoWedinstudio from "@assets/logo-wedinstudio.png";
...
function createReceiptPdf(receipt: {
  ...
  const text = (value: string, x: number, y: number, size: number, color = "0.12 0.16 0.22") =>
    `BT /F1 ${size} Tf ${color} rg ${x} ${y} Td (${pdfSafeText(value)}) Tj ET`;
  const rows = [
    ["Invoice ID", receipt.invoiceId],
    ["Invitation", receipt.invitationName],
    ["Package", receipt.packageName],
    ["Amount", receipt.amount],
    ["Payment date", receipt.paymentDate],
    ["Status", receipt.status],
    ...(receipt.gateway ? [["Gateway", receipt.gateway]] : []),
  ];
  const commands = [
    "q",
    "0.96 0.97 0.98 rg",
    ...
    "0.12 0.16 0.22 rg",
    "40 735 515 67 re f",
    text("Wedinstudio", 58, 765, 24, "1 1 1"),
    text("PAYMENT RECEIPT", 58, 744, 10, "0.78 0.83 0.89"),
    text("Thank you for your payment", 58, 690, 12, "0.35 0.39 0.45"),
    "0.84 0.86 0.89 RG",
    "40 675 m 555 675 l S",
    ...rows.flatMap(([label, value], index) => {
      const y = 635 - index * 48;
      return [
        text(label, 58, y, 9, "0.42 0.46 0.52"),
        text(value, 205, y, label === "Amount" ? 13 : 11, label === "Amount" ? "0.07 0.45 0.30" : "0.12 0.16 0.22"),
        ...(index < rows.length - 1 ? ["0.91 0.92 0.94 RG", `58 ${y - 17} m 537 ${y - 17} l S`] : []),
      ];
    }),
    text("This receipt was generated by Wedinstudio.", 58, 245, 9, "0.48 0.51 0.56"),
    text("Keep this document for your records.", 58, 228, 9, "0.48 0.51 0.56"),
  ].join("\n");
...
export default function DashboardPage() {
  ...
  const downloadReceipt = (payment: PaymentHistoryItem) => {
    ...
    const receipt = createReceiptPdf({
      invoiceId,
      invitationName,
      packageName: payment.packageName || "Wedding invitation",
      amount: `RM ${Number(payment.amount || 0).toFixed(2)}`,
      paymentDate: date,
      status: "Paid",
      gateway: payment.paymentGateway || undefined,
    });
    const url = URL.createObjectURL(receipt);
    const link = document.createElement("a");
    link.href = url;
    link.download = `${invoiceId}-receipt.pdf`;
    link.click();
  ...
  /** Returns a list of human-readable field names that must be filled before payment. */
  const missingPayFields = (card: Invitation): string[] => {
  ...
  const startPayment = async (input: { invitationId?: number; orderId?: number }) => {
    // Pre-payment validation: names + event date must be set before checkout so
    // the shareable URL is name-based from the moment the invitation goes live.
