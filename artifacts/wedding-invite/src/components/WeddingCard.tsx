    ...
    <div className="relative w-full mx-auto" style={{ maxWidth }}>
      ...
      <section className="relative z-10">
        ...
        <div className="relative z-10 flex flex-col items-center gap-[50px] pt-16 pb-28 px-[60px]">
          ...
          <RevealOnScroll>
          {/* Invitation Text — explicit per-gap spacing; no space-y on container */}
          <div className="w-full max-w-sm text-center flex flex-col">
            ...
            {/* OrnamentDivider → Parents: 12px | Parents internal: 6px */}
            {(brideParents || groomParents) && (
              <div className="mt-3 space-y-[6px]">
                {groomParents && (
                  <p className="font-medium" style={{ fontFamily: bodyFontFamily, fontSize: "var(--greeting-font-size, 1rem)", color: "var(--greeting-color, hsl(var(--primary)))" }} dangerouslySetInnerHTML={{ __html: sanitizeHtml(groomParents) }} />
                )}
                {brideParents && groomParents && (
                  <p className="font-medium" style={{ fontFamily: bodyFontFamily, fontSize: "var(--greeting-font-size, 1rem)", color: "var(--greeting-color, hsl(var(--primary)))" }}>&amp;</p>
                )}
                {brideParents && (
                  <p className="font-medium" style={{ fontFamily: bodyFontFamily, fontSize: "var(--greeting-font-size, 1rem)", color: "var(--greeting-color, hsl(var(--primary)))" }} dangerouslySetInnerHTML={{ __html: sanitizeHtml(brideParents) }} />
                )}
              </div>
            )}
            {/* Parents → Invitation message: 40px */}
            <p className="mt-10 text-foreground/70 leading-relaxed" style={{ fontFamily: bodyFontFamily, fontSize: "var(--greeting-font-size, 1rem)" }} dangerouslySetInnerHTML={{ __html: sanitizeHtml(invitationText) }} />
